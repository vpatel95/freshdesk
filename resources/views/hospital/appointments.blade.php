							...
							<ul id="gallery_grid" class="galMix list">
								<li>
									<div class="gal_sort_list clearfix">
										...
										<div class="meta disease">Disease</div>
								...
								@foreach($appointment as $ap)
									<li class="mix user_0 travel business" data-name="Image 1" data-timestamp="1452898800">
										<a href="{{ route('hospital.appointment', $ap->u_id) }}">
											<div class="meta name">
												<h2 class="gal_title">{{ App\UserDetail::find($ap->u_id)->name }}</h2>
												<span class="text-muted">User Id : {{ $ap->u_id }}</span>
											</div>
										</a>
										<div class="meta disease">{{ $ap->disease }}</div>
										<div class="meta date">{{ $ap->appointment_date }}</div>
										<div class="meta category">
											<span class="label label-default">Cardiology</span>
										</div>
									</li>
								@endforeach
							</ul>
